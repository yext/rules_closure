...
func TestIsJsLibrary(t *testing.T) {
	var tests = []struct {
		...
		{"closure_js_test", false},
		{"closure_jsx_library", true},
		{"closure_jsx_test", false},
...
func TestIsJsTest(t *testing.T) {
	var tests = []struct {
		in  string
		out bool
	}{
		{"closure_js_library", false},
		{"closure_js_test", true},
		{"closure_jsx_library", false},
		{"closure_jsx_test", true},
		{"closure_js_thirdparty_library", false},
		{"closure_js_template_library", false},
		{"closure_jsx_template_library", false},
	}
	for _, test := range tests {
		actual := isJsTest(test.in)
		if actual != test.out {
			t.Errorf("%v: expected %v, got %v", test.in, test.out, actual)
		}
	}
