...
`,
		...
		// NOTE: This case fails against the version of Gazelle pulled in by WORKSPACE.
		// See the fix in https://github.com/bazelbuild/bazel-gazelle/pull/784
		//
		// 		{
...
func runGazelle(wd string, args []string) error {
	...
	// There doesn't appear to be any platform-independent way to get the path
	// to the gazelle_js binary declared in this test's data attribute.
	// TODO: Investigate adding a cfg = "host" to gazelle_binary.
	gazellePath := path.Join(oldWd, "darwin_amd64_stripped/gazelle_js")
	return run(gazellePath, args)
}

func run(gazellePath string, args []string) error {
